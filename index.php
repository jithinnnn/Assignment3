...
<div class="container">
<div class="row">
    <div class="col-lg-4 droppable">
        <h2 class="heading">To Do</h2>
        <table id="todo-table" class="table table-success fs-6">
            <thead>
                <tr>
                    <th scope="col">#</th>
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_array($tasks_todo)) { ?>
                    <tr class="draggable" data-task-id="<?php echo $row['id']; ?>">
  ...
  .dropdown:hover .dropbtn {
    ...
    <div class="col-lg-4 droppable">
        ...
        <table id="progress-table" class="table table-warning fs-6">
            <thead>
                <tr>
                    <th scope="col">#</th>
                    <th scope="col">Task</th>
    ...
    <div class="col-lg-4 droppable">
        ...
        <table id="done-table" class="table table-info fs-6">
            <thead>
                <tr>
                    <th scope="col">#</th>
                    <th scope="col">Task</th>
...
$(document).ready(function(){
    ...
    $(".droppable").droppable({
        ...
        drop: function(event, ui) {
            // Get the dropped row's HTML
            var droppedRowHTML = ui.draggable.html();
            var taskId = ui.draggable.data('task-id');
            var tableName = $(this).attr('id');
            var status = '';

            // Determine the status based on the table ID
            if (tableName === 'todo-table') {
                status = 'To Do';
            } else if (tableName === 'progress-table') {
                status = 'In Progress';
            } else if (tableName === 'done-table') {
                status = 'Done';
            }
            ...
            // Append the HTML to the table's tbody
            $(this).find("tbody").append("<tr class='draggable' data-task-id='" + taskId + "'>" + droppedRowHTML + "</tr>");
            
            // Re-initialize draggable on the dropped row
            $(this).find(".draggable[data-task-id='" + taskId + "']").draggable({
                helper: "clone",
                cursor: "move"
            });

            // Remove the original row if needed
            ui.draggable.remove();
        }
    });

    // Move tasks to their respective tables on page load
    $(".draggable").each(function() {
        var taskId = $(this).data('task-id');
        var status = localStorage.getItem(taskId);
